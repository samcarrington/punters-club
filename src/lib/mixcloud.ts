...
export const normalizeShow = (
  ...
  const pictures =
    typeof show.pictures === "object" && show.pictures
      ? (show.pictures as Record<string, string>)
      : undefined;
  const artwork =
    pictures?.large ??
    pictures?.["320wx320h"] ??
  ...
  const tags = Array.isArray(show.tags)
    ? show.tags
        .map((tag) => {
          ...
          const name =
            typeof tagRecord.name === "string" ? tagRecord.name : undefined;
          if (!name) return null;
          return {
            name,
        ...
        .filter(
          (tag): tag is { name: string; url?: string; key?: string } =>
            tag !== null,
        )
    : undefined;
  ...
  return {
    ...
    description:
      ...
      source.description ??
      (show.description as string | undefined),
    ...
    durationSeconds:
      typeof show.audio_length === "number"
        ...
        : typeof show.duration === "number"
          ...
          : undefined,
    playCount:
      typeof show.play_count === "number" ? show.play_count : undefined,
    favoriteCount:
      typeof show.favorite_count === "number" ? show.favorite_count : undefined,
    commentCount:
      typeof show.comment_count === "number" ? show.comment_count : undefined,
    tags,
    pictures,
    artwork,
    user:
      typeof show.user === "object" && show.user
        ? (show.user as Show["user"])
        : undefined,
  };
};
