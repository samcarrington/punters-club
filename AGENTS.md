...
- Use npm and `package-lock.json`; this is not a pnpm workspace/monorepo.
- Astro dependency requires a current Node line (`^20.19.0 || >=22.12.0` via lockfile packages).

## Commands
- Dev server: `npm run dev`.
- Full production build: `npm run build` (`npm run enrich && astro build`).
- Build without refreshing generated Mixcloud/Spotify metadata: `npx astro build`.
...
- `npm run build` can update generated play counts/artwork metadata. If the task is UI-only, restore unrelated generated JSON churn or use `npx astro build` for validation.
